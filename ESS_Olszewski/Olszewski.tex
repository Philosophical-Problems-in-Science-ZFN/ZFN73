...
We will consider an infinite two-person game with complete information, which we denote by the symbol G(A), where A${\subset}$${\omega}$\textsuperscript{${\omega}$}, that is A~is a~subset of the set of all infinite sequences of natural numbers with zero. The symbol ${\omega}$\textsuperscript{{\textless}${\omega}$} denotes the set of all finite sequences of natural numbers. The \textit{empty sequence} is denoted by %
${\langle}{\rangle}$, and the \textit{length} od the finite sequence s~by {\textbar}s{\textbar}. We also have two players: Player \textbf{I} and Player \textbf{II}, who take turns making \textit{moves}, i.e. choices of natural numbers.}\textbf{
...
\begin{supertabular}{|m{2.5479999cm}|m{10.394cm}|}
\hline
Player I: &
x\textsubscript{0} x\textsubscript{1} x\textsubscript{2} … …\\\hline
Player II: &
y\textsubscript{0} y\textsubscript{1} y\textsubscript{2} … ...\\\hline
\end{supertabular}
\end{flushleft}
Table 2. A~graphic representation of the game.

We use the moves of both players to create one infinite sequence of the form: z~:= ${\langle}$x\textsubscript{0}, y\textsubscript{0}, x\textsubscript{1}, y\textsubscript{1}, … ${\rangle}$, which we call \textit{a} \textit{play} in \textit{a~game} G(A). Definitions of players' strategies play a~key role:

\textbf{\textcolor{black}{Definition 1.}}\textcolor{black}{ Strategy }${\sigma}$\textcolor{black}{ for Player }\textbf{\textcolor{black}{I}}\textcolor{black}{ is the function:}

\textcolor{black}{${\sigma}$: \{ s${\in}{\omega}$}\textcolor{black}{\textsuperscript{{\textless}${\omega}$}}\textcolor{black}{ : {\textbar}s{\textbar} is even\} ${\rightarrow}$ ${\omega}$.[F08C?]}
...
x$*{\tau}$ = ${\langle}$x\textsubscript{0}, y\textsubscript{0}, x\textsubscript{1}, y\textsubscript{1}, x\textsubscript{2}, y\textsubscript{2}, …${\rangle}$;
...
\begin{itemize}
\item Strategy ${\sigma}$ is \textbf{a} \textbf{winning} s\textbf{trategy} for Player \textbf{I} in Game G(A) iff for all y${\in}{\omega}$\textsuperscript{${\omega}}$, we have: ${\sigma}*$y ${\in}$ A.
\item Strategy ${\tau}$ is \textbf{a} \textbf{winning} s\textbf{trategy} for Player \textbf{II} in Game G(A) iff for all x${\in}{\omega}$\textsuperscript{${\omega}}$, we have: x$*$${\tau}$ ${\not\in}$ A.
\end{itemize}
With the above definitions, we can formulate the axiom of determinacy (\textbf{AD}}):\textbf{

\begin{itemize}
\item For each set A${\subset}{\omega}$\textsuperscript{${\omega}$}, Game G(A) is determined i.e. exactly one of the players has a~winning strategy for G(A).[F08C?]
\end{itemize}
...
The meager and comeager sets relate to Baire space [F077?]\textsuperscript{[F077?]}. Intuitively ``comeager sets are large. They form a~filter, are dense, uncountable, and are closed under countable intersections. Meager sets are small. They form an ideal, and countable sets are meager.''
